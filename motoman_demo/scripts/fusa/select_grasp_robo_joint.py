...
#--------------- target_pose, place_poseから、つかめる grasp を厳選する --------------------------------------------
        ...
        print "Rr rpy  = %f, %f, %f" %(trans_roll, trans_pitch, trans_yaw)
        ...
        for num in range(1,7):
            ...
            Rg = Rh.dot(Rt)
            Rg_rpy = list(tf.transformations.euler_from_matrix(Rg, 'rzyx'))
            print grasp_roll , grasp_pitch , grasp_yaw
            print Rg_rpy
            for i in range(len(Rg_rpy)):    #小さい変な値を０にする
                if -0.01 < Rg_rpy[i] < 0.01:
                    Rg_rpy[i] = 0
            print "clearn rpy  " + str(Rg_rpy)
            Rg_rpy_np = np.rad2deg(Rg_rpy)
            Rg_rpy_list = Rg_rpy_np.tolist() #numpy_arry型からlist型にする
